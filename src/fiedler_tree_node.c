...
static BfSize const LEAF_SIZE_THRESHOLD = 64;
...
// enum NodalDomainType {UNKNOWN, POSITIVE, NEGATIVE, BOUNDARY, ISOLATED};
...
//   /** Flood fill positive nodes starting from maximum: */
...
//   /** Flood fill negative nodes starting from minimum: */
...
//   for (BfSize i = 0; i < bfTrimeshGetNumVerts(trimesh); ++i) {
//     if (nodalDomainType[i] == UNKNOWN) {
//       bfRealArraySetValue(phiFiedler, i, BF_NAN);
//       nodalDomainType[i] = ISOLATED;
//     }
//   }
...
static void splitTrimesh(BfTrimesh const *trimesh, BfRealArray const *phiFiedler,
  ...
  if (bfRealArrayGetSize(phiFiedler) != numVerts)
    RAISE_ERROR(BF_ERROR_INVALID_ARGUMENTS);

  /* We need to do some preprocessing here. If we compute the Fiedler
   * vector very inaccurately, we may not have a clean separation into
   * two nodal domains. */

  // BfSize iZero = bfTrimeshGetNumVerts(trimesh);
  // BF_ASSERT(iZero > 0);
  // while (iZero > 0 && fabs(bfRealArrayGetValue(phiFiedler, iZero - 1)) <= tol)
  //   --iZero;
  // BF_ASSERT(iZero > 0);

  BfRealArray *phiFiedlerAdjusted = bfRealArrayNewWithCapacity(numVerts);
  HANDLE_ERROR();

  for (BfSize i = 0; i < numVerts; ++i) {
    BfReal phi = bfRealArrayGetValue(phiFiedler, i);

    BfSize numNeighbors = bfTrimeshGetNumVertexNeighbors(trimesh, i);

    BfReal *phiNeighbor = bfMemAlloc(numNeighbors, sizeof(BfReal));
    HANDLE_ERROR();

    for (BfSize j = 0; j < numNeighbors; ++j) {
      BfSize iNb = bfTrimeshGetVertexNeighbor(trimesh, i, j);
      phiNeighbor[j] = bfRealArrayGetValue(phiFiedler, iNb);
    }

    bool hasPosNeighbor = false;
    bool hasNegNeighbor = false;
    for (BfSize j = 0; j < numNeighbors; ++j) {
      if (phiNeighbor[j] > tol) hasPosNeighbor = true;
      if (phiNeighbor[j] < -tol) hasNegNeighbor = true;
    }

    if (fabs(phi) > tol) {
      if ((phi < 0 && !hasNegNeighbor) || (phi > 0 && !hasPosNeighbor)) {
        BfReal phiMean = 0;
        for (BfSize j = 0; j < numNeighbors; ++j)
          phiMean += phiNeighbor[j];
        phiMean /= numNeighbors;
        if (phi < -tol) BF_ASSERT(phiMean > tol);
        if (phi > tol) BF_ASSERT(phiMean < -tol);
        phi = phiMean;
      }
    }
    bfRealArrayAppend(phiFiedlerAdjusted, phi);
  }

  /* Extract the two nodal domains determined by `phiFiedler`. */

  BfTrimesh *submesh1 = bfTrimeshGetLevelSetSubmesh(trimesh, phiFiedlerAdjusted, tol, permMask, perm1Ptr);
  HANDLE_ERROR();

  bfRealArrayNegate(phiFiedlerAdjusted);

  BfTrimesh *submesh2 = bfTrimeshGetLevelSetSubmesh(trimesh, phiFiedlerAdjusted, tol, permMask, perm2Ptr);
  HANDLE_ERROR();

  //   // DELETE THE SUBPERMS INDS FROM THE PARENT INDS AND FIND THE
  //   // MISSING IND. THEN RE-RUN TO FIGURE OUT WHERE IT WENT.

  //   for (BfSize j = 0; j < MAX_NUM_CHILDREN; ++j)
  //     for (BfSize k = 0; k < bfSizeArrayGetSize(subperm[j]); ++k)
  //       permMask[i0 + bfSizeArrayGet(subperm[j], k)] = false;
  //   for (BfSize i = i0; i < i1; ++i) {
  //     if (permMask[i]) {
  //       printf("bad guy: %lu\n", i - i0);
  //       printf("* phi[%lu] = %g\n", i - i0, bfRealArrayGetValue(phiFiedler, i-i0));
  //       printf("* neighboring Fiedler values:\n");
  //       for (BfSize j=0; j<bfTrimeshGetNumVertexNeighbors(trimesh, i-i0); ++j) {
  //         BfSize iNb = bfTrimeshGetVertexNeighbor(trimesh, i-i0, j);
  //         printf("  - phi[%lu] = %g\n", iNb, bfRealArrayGetValue(phiFiedler, iNb));
  //       }
  //     }
  //   }
  // }
  ...
  bfRealArrayDeinitAndDealloc(&phiFiedlerAdjusted);
